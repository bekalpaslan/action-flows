...
| 4 | Dark Theme CSS Variables | All color values use CSS custom properties (--color-bg, --color-text, etc.) from theme provider. No hardcoded color values (no `#fff`, `rgb(255,255,255)`, etc.). Dark theme colors distinct and readable (contrast ratio >= 4.5:1 for text). | **CRITICAL** |
...
| 13 | Control Buttons State Binding | Pause button enabled only when session RUNNING, disabled otherwise. Resume button enabled only when session PAUSED, disabled otherwise. Cancel button enabled for RUNNING or PAUSED sessions. Retry enabled only for FAILED steps. Skip enabled for PENDING or FAILED steps. Button labels clear and descriptive. | **CRITICAL** |
