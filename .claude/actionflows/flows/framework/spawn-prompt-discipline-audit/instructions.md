...
- rule: Spawn prompts must provide ONLY (1) agent.md read instruction, (2) project config injection, (3) task-specific inputs. Anything else — subagent identity warnings, "do not delegate", "do not read ORCHESTRATOR.md", restated format rules, restated constraints — is a violation. If a subagent needs that info, it belongs in agent.md.
- output: Per-file list of {path, line range, violating text, violation type, suggested fix (strip | migrate-to-agent.md)} plus cross-flow patterns.
```
...
- **Lint hook candidate:** Grep for `You are a spawned subagent executor` in `flows/**/instructions.md` as a pre-commit or framework-health check.
